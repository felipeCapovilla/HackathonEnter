...
function Monitoring() {
  ...
  const data = resource.data;
  return <article className="panel">
    <div className="panel-heading"><span className="panel-icon"><Icon name="chart" /></span>
      <div><h2>Monitoramento do banco</h2><p>Decisões emitidas e aderência à política.</p></div></div>
    <ErrorNotice message={resource.error} onRetry={resource.reload} />
    {resource.loading && <p role="status">Carregando…</p>}
    {data && <div className="monitor">
      <Metric label="Análises" value={data.total_analyses} />
      <Metric label="Decisões" value={data.total_lawyer_decisions} />
      <Metric label="Aderência" value={rate(data.adherence_rate)} hint="Decisões que seguiram a recomendação" />
    </div>}
  </article>;
}
